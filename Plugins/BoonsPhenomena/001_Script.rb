...
class Phenomenon
  ...
  def initialize(types)
    Console.echo_p(_INTL("Initializing for map with types: `{1}`",types))
    @x = nil
    @y = nil
    @types = types
  ...
  def generate!
    Kernel.echo(_INTL("Generating phenomena..."))
	...
	# Note: For cave and bird tiles, PassableStrict will return true if the player is debug walking. Phenomena spawning out of bounds while the player is debug walking is not a bug, as the end user can't debug walk.
    if phenomenon_tiles.length == 0
	  Kernel.echoln("")
      Console.echo_warn(_INTL("A phenomenon is set up but no compatible tiles are available! Phenomena: {1}",@types))
      pbPhenomenonCancel
    else
      ...
      @active = true
	  Console.echo_p(_INTL("Placed `{3}` at [${1},{2}$]",@x,@y))
    end
  end

  def activate!
    Console.echo_p("Activating phenomenon for `#{@type}`")
    encounter = nil
    item = nil
