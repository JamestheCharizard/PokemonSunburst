...
if Settings::BW_POKEDEX_COMPAT
  class PokemonPokedexInfo_Scene
    ...
    def drawPageInfo
      ...
      if @brief
        textpos = [
           [_INTL("Pokémon Registration Complete"), 82, 6, 0, Color.new(255, 255, 255), Color.new(165, 165, 173)],
           [_INTL("{1}{2} {3}", indexText, " ", species_data.name),
              272, 62, 0, Color.new(82, 82, 90), Color.new(165, 165, 173)],
           [_INTL("Height"), 288, 178, 0, base, shadow],
           [_INTL("Weight"), 288, 208, 0, base, shadow]
        ]
      else
        textpos = [
           [_INTL("{1}{2} {3}", indexText, " ", species_data.name),
              272, 24, 0, Color.new(82, 82, 90), Color.new(165, 165, 173)],
      ...
      if $Trainer.owned?(@species)
        if @brief
          textpos.push([_INTL("{1} Pokémon", species_data.category), 376, 98, 2, base, shadow])
        else
        ...
        if System.user_language[3..4] == "US"
          ...
          if @brief
            textpos.push([_ISPRINTF("{1:d}'{2:02d}\"", inches / 12, inches % 12), 490, 178, 1, base, shadow])
          else
            textpos.push([_ISPRINTF("{1:d}'{2:02d}\"", inches / 12, inches % 12), 490, 140, 1, base, shadow])
          end
          if @brief
            textpos.push([pounds, 490, 208, 1, base, shadow])
          else
            textpos.push([pounds, 490, 170, 1, base, shadow])
          end
        else
          kilograms = (@gmax) ? _INTL("????.? kg") : _ISPRINTF("{1:.1f} kg", weight / 10.0)
          if @brief
            textpos.push([_ISPRINTF("{1:.1f} m", height / 10.0), 490, 178, 1, base, shadow])
          else
            textpos.push([_ISPRINTF("{1:.1f} m", height / 10.0), 490, 140, 1, base, shadow])
          end
          if @brief
            textpos.push([kilograms, 490, 208, 1, base, shadow])
          else
            textpos.push([kilograms, 490, 170, 1, base, shadow])
          end
        end
        ...
        if @brief
          ...
          overlay.blt(386, 132, @typebitmap.bitmap, type2rect) if type1 != type2
        else
          overlay.blt(286, 94, @typebitmap.bitmap, type1rect)
          overlay.blt(386, 94, @typebitmap.bitmap, type2rect) if type1 != type2
        end
      else
        textpos.push([_INTL("????? Pokémon"), 274, 60, 0, base, shadow])
        if System.user_language[3..4] == "US"
          textpos.push([_INTL("???'??\""), 490, 140, 1, base, shadow])
          textpos.push([_INTL("????.? lbs."), 488, 170, 1, base, shadow])
        else
          textpos.push([_INTL("????.? m"), 488, 140, 1, base, shadow])
    ...
    def drawPageForms
      ...
      textpos = [
         [_INTL("Forms"),58,8,0,Color.new(255,255,255),Color.new(115,115,115)],
