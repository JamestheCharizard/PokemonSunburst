  ...
  class DataBoxEBDX  <  SpriteWrapper
    def applyMetrics
      ...
      @data = {
        ...
        "terastal" => {:x => @playerpoke ? -10 : 206, :y => -18, :z => 1},
    ...
    def setUp
      ...
      @sprites["terastal"] = Sprite.new(@viewport)
      @sprites["terastal"].z = self.getMetric("terastal", :z)
      @sprites["terastal"].mirror = @playerpoke
      @sprites["terastal"].ex = self.getMetric("terastal", :x)
      @sprites["terastal"].ey = self.getMetric("terastal", :y)
      
      @ultraBmp   = pbBitmap(@path + "symUltra")
      @dynamaxBmp = pbBitmap(@path + "symDynamax")
      @teraBmp = pbBitmap(@path + "symTeraTypes")
    end
    ...
    def refresh
      ...
      pbDrawOutlineText(@sprites["textName"].bitmap,18-o,11,@sprites["textName"].bitmap.width-40,@sprites["textName"].bitmap.height,str,color,shadow,0)
      if $game_switches[Settings::MAXRAID_SWITCH] && !@battler.is_a?(PokeBattle_FakeBattler) && @battler.effects[PBEffects::MaxRaidBoss]
        ...
        pbDrawOutlineText(@sprites["timercount"].bitmap,-8,15,@sprites["timercount"].bitmap.width-40,@sprites["timercount"].bitmap.height,"#{turncount}",base[turncolor],shadow,2)
        pbDrawOutlineText(@sprites["kocount"].bitmap,19,15,@sprites["kocount"].bitmap.width-40,@sprites["kocount"].bitmap.height,"#{kocount}",base[kocolor],shadow,2)
        ...
        pbDrawOutlineText(@sprites["textName"].bitmap,x-o,11,@sprites["textName"].bitmap.width-40,@sprites["textName"].bitmap.height,str,color,Color.new(0,0,0,125),0)
        # writes the Pokemon's level
        str = "Lv.#{@battler.level}"
        pbDrawOutlineText(@sprites["textName"].bitmap,18+o,11,@sprites["textName"].bitmap.width-40,@sprites["textName"].bitmap.height,str,Color.white,Color.new(0,0,0,125),2)
      end
      ...
      if @battler.terastalized?
        @sprites["terastal"].bitmap = Bitmap.new(34,34)
        typerect = Rect.new(0, 34*GameData::Type.get(@battler.pokemon.tera_type).id_number, 34, 34)
        @sprites["terastal"].bitmap.blt(0,0,@teraBmp,typerect)
      elsif @sprites["terastal"].bitmap
        @sprites["terastal"].bitmap.clear
        @sprites["terastal"].bitmap = nil
      end
      self.updateHpBar
      self.updateExpBar
    end
