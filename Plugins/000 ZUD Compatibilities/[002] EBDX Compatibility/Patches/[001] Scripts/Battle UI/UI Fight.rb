...
if Settings::EBDX_COMPAT
  class PokeBattle_Scene
    ...
    def pbFightMenu(idxBattler,megaEvoPossible = false,
      ...
      @fightWindow.teraButton  if terastalPossible && @battle.pbCanTerastalize?(idxBattler)
      # last chosen move
      moveIndex = 0
  ...
  class FightWindowEBDX
    ...
    def initialize(*args)
      ...
      @teraButton.bitmap = pbBitmap(@path + @dynaImg)
      @teraButton.z = 101
      @teraButton.src_rect.width /= 2
      @teraButton.center!
      @teraButton.x = 30
      @teraButton.y = @viewport.height - @background.bitmap.height/2 + 100
      @showTerastal = false
    end
    ...
    def applyMetrics
      ...
      for data in [d2, d7, d6, d5, d1]
        if !data.nil?
          ...
          @teraImg = data[:TERABUTTONGRAPHIC] if data.has_key?(:TERABUTTONGRAPHIC) && data[:TERABUTTONGRAPHIC].is_a?(String)
    ...
    def generateButtons
      ...
      for i in 0...@nummoves
        ...
        pbDrawOutlineText(@button["#{i}"].bitmap, 198, 18, 196, 42,"#{short_name}", Color.white, baseColor, 1)
        if movedata.zMove?
          actualpp = 1
          totalpp  = 1
        ...
        pp = "#{actualpp}/#{totalpp}"
        pbDrawOutlineText(@button["#{i}"].bitmap, 0, 56, 191, 26, pp, Color.white, baseColor, 2)
        pbSetSystemFont(@button["#{i}"].bitmap)
        text = [[short_name, 99, 12, 2, baseColor, Color.new(0, 0, 0, 24)]]
    ...
    def teraButton;  @showTerastal    = true; end
    ...
    def teraButtonTrigger
      @teraButton.src_rect.x += @teraButton.src_rect.width
      @teraButton.src_rect.x = 0 if @teraButton.src_rect.x > @teraButton.src_rect.width
      @teraButton.src_rect.y = -4
    end
    ...
    def update
      ...
      if @showTerastal
        @teraButton.y -= 10 if @teraButton.y > @viewport.height - @background.bitmap.height/2
        @teraButton.src_rect.y += 1 if @teraButton.src_rect.y < 0
      end
      if @oldindex != @index
