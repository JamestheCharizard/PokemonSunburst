...
def pbRaidRankLists
  ...
  GameData::Species.each do |sp|
    ...
    rank1.push(sp.id) if bst <= 360                 && !banRank1 && !exception
    rank2.push(sp.id) if (bst < 480 && bst > 360)   && !banRank2 && !exception
...
def pbMaxRaidMovelists(pkmn,rental=false)
  ...
  blacklist  = ["0CE","0DE", # Sky Drop, Dream Eater
                ...
                "0C2","0E0","80C", # Recharge moves, Self-KO moves
...
Events.onWildPokemonCreate += proc { |_sender, e|
  ...
  if $game_switches[Settings::MAXRAID_SWITCH]
    ...
    #---------------------------------------------------------------------------
    rank = 1
    rank = 2  if bosslevel>=Settings::MAXRAID_LEVEL_RANKS[2]
    rank = 3  if bosslevel>=Settings::MAXRAID_LEVEL_RANKS[3]
    rank = 4  if bosslevel>=Settings::MAXRAID_LEVEL_RANKS[4]
    rank = 5  if bosslevel>=Settings::MAXRAID_LEVEL_RANKS[5]
    rank = 6  if bosslevel>=Settings::MAXRAID_LEVEL_RANKS[6]
    dlvl = 5
    dlvl = 10 if rank==2
    dlvl = 20 if rank==3
    dlvl = 30 if rank==4
