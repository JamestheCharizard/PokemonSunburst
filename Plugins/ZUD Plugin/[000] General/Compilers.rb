...
#===============================================================================
  def compile_trainers(path = "PBS/trainers.txt")
    ...
    pbCompilerEachPreppedLine(path) { |line, line_no|
      if line[/^\s*\[\s*(.+)\s*\]\s*$/]
        ...
        when "Pokemon"
		  ...
		  when "TeraType"
            if property_value[/^\w+$/]
              current_pkmn[:tera_type] = property_value.to_sym
            elsif !GameData::Type.exists?(property_value.to_sym)
              raise _INTL("Value {1} isn't a defined type.\r\n{2}", property_value, FileLineData.linereport)
            else
              current_pkmn[line_schema[0]] = property_value.to_sym
            end				
          else
        ...
        when 3
          ...
          line_data = pbGetCsvRecord(line, line_no,
             [0, "evEEEEEUEUBEUUSBUUBBE", :Species, nil, :Item, :Move, :Move, :Move, :Move, nil,
                                      {"M" => 0, "m" => 0, "Male" => 0, "male" => 0, "0" => 0,
                                      "F" => 1, "f" => 1, "Female" => 1, "female" => 1, "1" => 1},
                                      nil, nil, :Nature, nil, nil, nil, nil, nil, nil, nil, nil, :Type])
          current_pkmn = {
            :species => line_data[0]
          }
          ...
          current_pkmn[:tera_type]    = line_data[20] if line_data[20]
    ...
    # Gather pokemon nicknames for translating
    pokemon_nicknames = []
    GameData::Trainer.each do |trainer|
      trainer.pokemon.each do |pkmn|
        pokemon_nicknames.push(pkmn[:name]) if !nil_or_empty?(pkmn[:name])
      end
    end
	...
	MessageTypes.setMessagesAsHash(MessageTypes::PokemonNicknames, pokemon_nicknames)
    Graphics.update
  end
    ...
    File.open("PBS/trainers.txt", "wb") { |f|
      ...
      GameData::Trainer.each do |trainer|
        ...
        trainer.pokemon.each do |pkmn|
          ...
          f.write(sprintf("    TeraType = %d\r\n", pkmn[:tera_type])) if pkmn[:tera_type]
