...
class PokeBattle_Battle
  ...
  def pbCanZMove?(idxBattler)
    ...
    return false if battler.hasTerastal?                     # No Z-Moves if Terastalization is available first.
  ...
  def pbCanDynamax?(idxBattler)
	...
	return false if battler.terastalized?                     # No Dynamax for Terastalized Pokémon.
  ...
  def pbUseZMove(idxBattler,move,crystal)
	...
	# Update Game_Stats
	$stats.zmove_use_count += 1 if battler.pbOwnedByPlayer?
  ...
  def pbUltraBurst(idxBattler)
	...
	# Update Game_Stats - treat ultraburst as mega evolution
  ...
  def pbDynamax(idxBattler)
	...
	# Update Game_Stats
	$stats.times_dynamaxed += 1 if battler.pbOwnedByPlayer?
...
class PokeBattle_AI
  def pbDefaultChooseEnemyCommand(idxBattler)
    ...
    @battle.pbRegisterTerastal(idxBattler) if pbEnemyShouldTerastalize?(idxBattler)
  ...
  #-----------------------------------------------------------------------------
  # Terastal - The AI will only use Dynamax on their Trainer Ace Pokemon.
  #-----------------------------------------------------------------------------
  def pbEnemyShouldTerastalize?(idxBattler)
    battler = @battle.battlers[idxBattler]
    if @battle.pbCanTerastalize?(idxBattler) && battler.pokemon.trainerAce?
      PBDebug.log("[AI] #{battler.pbThis} (#{idxBattler}) will Terastalize")
      return true
    end
    return false
  end
