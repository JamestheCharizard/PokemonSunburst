...
#-------------------------------------------------------------------------------
class PokeBattle_ZMove_Weather < PokeBattle_ZMove
  def initialize(battle,move,newMove)
    super
    @weatherType = :None
  end
  ...
  def pbEffectGeneral(user)
    if ![:HarshSun, :HeavyRain, :StrongWinds, @weatherType].include?(@battle.field.weather) &&
       ![:Glitch].include?(@battle.field.terrain)
...
# Snowstorm Pounce
#===============================================================================
# Considered super effective on Water-types. Sets hail if not already set.
#-------------------------------------------------------------------------------
class PokeBattle_Move_Z100 < PokeBattle_ZMove_Weather
  def initialize
    super
    @weatherType = :Snow
  end
...
#===============================================================================
# Spiral Needle Slammer
#===============================================================================
# Sets Grassy Terrain.
#-------------------------------------------------------------------------------
class PokeBattle_Move_Z101 < PokeBattle_ZMove
  def pbAdditionalEffect(user,target)
    @battle.pbStartTerrain(user, :Grassy) if @battle.field.terrain!= :Grassy
  end
end 