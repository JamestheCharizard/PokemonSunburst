...
class PokeBattle_Battler
  ...
  def pbFaint(showMessage=true)
    if @battle.decision==0 && !pbOwnedByPlayer? && @effects[PBEffects::MaxRaidBoss]
      ...
      return if @fainted
	  # Check the apu pokemon's revive state
      if @effects[PBEffects::ApuRevival] > 0
        # Heal back to full
        pbRecoverHP(self.totalhp - self.hp) # Should be 0 but just in case
        @battle.pbDisplayBrief(_INTL("The Apu {1} recovered due to its aura!",pbThis))
        PBDebug.log("[Pokémon triggered Apu Revival] #{pbThis} (#{@index})")
        # Reduce ApuRevival state
        @effects[PBEffects::ApuRevival] -= 1
        # Exit faint script
        return
	  end
	  self.unmax if dynamax?  # Reverts Dynamax upon fainting.
	  self.unterastalize if terastalized? # Reverts Terastalization upon fainting.
