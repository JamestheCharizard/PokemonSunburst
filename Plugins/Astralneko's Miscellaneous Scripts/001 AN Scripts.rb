...
# Special item articles
# Used for "some Leftovers", "a pair of Loaded Dice", etc.
def anItemHasSpecialArticle?(item,quantity=1)
	alwaysHasSpecialArticle = [
		:LEFTOVERS,    :LOADEDDICE,  :WISEGLASSES,   :BLACKGLASSES,   :HONEY,
		:STARDUST,     :GROWTHMULCH, :DAMPMULCH,     :STABLEMULCH,    :GOOEYMULCH,
		:SHOALSALT,    :REDNECTAR,   :YELLOWNECTAR,  :PINKNECTAR,     :PURPLENECTAR,
		:SAFETYGOGGLES,:BRIGHTPOWDER,:PROTECTIVEPADS,:HEAVYDUTYBOOTS, :CHOICESPECS,
		:LIGHTCLAY,    :BLACKSLUDGE, :LUMINOUSMOSS,  :THROATSPRAY,    :ROOMSERVICE,
		:LAXINCENSE,   :FULLINCENSE, :LUCKINCENSE,   :PUREINCENSE,    :SEAINCENSE,
		:WAVEINCENSE,  :ROSEINCENSE, :ODDINCENSE,    :ROCKINCENSE,    :CHARCOAL,
		:NEVERMELTICE, :SOFTSAND,    :SILVERPOWDER,  :METALPOWDER,    :QUICKPOWDER,
		:REAPERCLOTH,  :WHIPPEDDREAM,:SACREDASH,     :PEWTERCRUNCHIES,:BERRYJUICE,
		:FRESHWATER,   :SODAPOP,     :LEMONADE,      :MOOMOOMILK,     :ENERGYPOWDER,
		:HEALPOWDER,   :MAXHONEY,    :PPUP,          :PPMAX,          :HPUP,
		:PROTEIN,      :IRON,        :CALCIUM,       :ZINC,           :CARBOS,
		:ARMORITEORE,  :DYNITEORE,   :AUSPICIOUSARMOR,:MALICIOUSARMOR,:BERSERKGENE,
		:MAXSOUP,      :MAXEGGS,     :MAXSCALES,      :MAXPLUMAGE
	]
	return true if GameData::Item.get(item).is_mail?
	return true if GameData::Item.get(item).is_z_crystal?
	return true if GameData::Item.get(item).is_grocery?
	return true if GameData::Item.get(item).is_poke_material? &&
	               GameData::Item.get(item).name == GameData::Item.get(item).name_plural
	return alwaysHasSpecialArticle.include?(item)
end

# Returns the special article for the above items
def anSpecialArticle(item,quantity=1)
  case item
  #----------------------------------------------------------------##
  when :LEFTOVERS,:MAXSOUP
    if quantity == 1
      return _INTL("a serving of")
    else
      return _INTL("{1} servings of",quantity)
    end
  #----------------------------------------------------------------##
  when :LOADEDDICE,:WISEGLASSES,:BLACKGLASSES,:SAFETYGOGGLES,:HEAVYDUTYBOOTS,
       :CHOICESPECS
    if quantity == 1
      return _INTL("a pair of")
    else
      return _INTL("{1} pairs of",quantity)
    end
  #----------------------------------------------------------------##
  when :PROTECTIVEPADS,:AUSPICIOUSARMOR,:MALICIOUSARMOR,:MAXSCALES
    if quantity == 1
      return _INTL("a set of")
    else
      return _INTL("{1} pairs of",quantity)
    end
  #----------------------------------------------------------------##
  when :HONEY,:REDNECTAR,:YELLOWNECTAR,:PINKNECTAR,:PURPLENECTAR,
       :LAXINCENSE,:FULLINCENSE,:LUCKINCENSE,:PUREINCENSE,:SEAINCENSE,
	   :WAVEINCENSE,:ROSEINCENSE,:ODDINCENSE,:ROCKINCENSE
    if quantity == 1
      return _INTL("a jar of")
    else
      return _INTL("{1} jars of",quantity)
    end
  #----------------------------------------------------------------##
  when :STARDUST,:GROWTHMULCH,:DAMPMULCH,:STABLEMULCH,:GOOEYMULCH,
       :BRIGHTPOWDER,:SOFTSAND,:METALPOWDER,:QUICKPOWDER,:SACREDASH,
	   :PEWTERCRUNCHIES
    if quantity == 1
      return _INTL("a bag of")
    else
      return _INTL("{1} bags of",quantity)
    end
  #----------------------------------------------------------------##
  when :CHARCOAL,:NEVERMELTICE,:ARMORITEORE,:DYNITEORE
    if quantity == 1
      return _INTL("a piece of")
    else
      return _INTL("{1} pieces of",quantity)
    end
  #----------------------------------------------------------------##
  when :MOOMOOMILK
    if quantity == 12
      return _INTL("a dozen bottles of")
    else
      return _INTL("a bottle of") if quantity == 1
      return _INTL("{1} bottles of",quantity)
    end
  #----------------------------------------------------------------##
  when :FRESHWATER,:SODAPOP,:LEMONADE,:THROATSPRAY,:PPUP,:PPMAX,
       :HPUP,:PROTEIN,:IRON,:CALCIUM,:ZINC,:CARBOS
    if quantity == 1
      return _INTL("a bottle of")
    else
      return _INTL("{1} bottles of",quantity)
    end
  #----------------------------------------------------------------##
  when :ENERGYPOWDER,:HEALPOWDER
    if quantity == 1
      return _INTL("a dose of")
    else
      return _INTL("{1} doses of",quantity)
    end
  #----------------------------------------------------------------##
  when :SHOALSALT,:SILVERPOWDER
    if quantity == 1
      return _INTL("a pile of")
    else
      return _INTL("{1} piles of",quantity)
    end
  #----------------------------------------------------------------##
  when :BERRYJUICE
    if quantity == 1
      return _INTL("a cup of")
    else
      return _INTL("{1} cups of",quantity)
    end
  #----------------------------------------------------------------##
  when :LIGHTCLAY
    if quantity == 1
      return _INTL("a lump of")
    else
      return _INTL("{1} lumps of",quantity)
    end
  #----------------------------------------------------------------##
  when :LUMINOUSMOSS
    if quantity == 1
      return _INTL("a clump of")
    else
      return _INTL("{1} clumps of",quantity)
    end
  #----------------------------------------------------------------##
  when :BLACKSLUDGE
    if quantity == 1
      return _INTL("a blob of")
    else
      return _INTL("{1} blobs of",quantity)
    end
  #----------------------------------------------------------------##
  when :REAPERCLOTH
    if quantity == 1
      return _INTL("a scrap of")
    else
      return _INTL("{1} scraps of",quantity)
    end
  #----------------------------------------------------------------##
  when :WHIPPEDDREAM
    if quantity == 1
      return _INTL("a dollop of")
    else
      return _INTL("{1} dollops of",quantity)
    end
  #----------------------------------------------------------------##
  when :MAXHONEY
    if quantity == 1
      return _INTL("a comb of")
    else
      return _INTL("{1} combs of",quantity)
    end
  #----------------------------------------------------------------##
  when :MAXEGGS
    if quantity == 1
      return _INTL("a clutch of")
    else
      return _INTL("{1} clutches of",quantity)
    end
  #----------------------------------------------------------------##
  when :MAXPLUMAGE
    if quantity == 1
      return _INTL("a bundle of")
    else
      return _INTL("{1} bundles of",quantity)
    end
  #----------------------------------------------------------------##
  when :ROOMSERVICE
    if quantity == 1
      return _INTL("a voucher for")
    else
      return _INTL("{1} vouchers for",quantity)
    end
  #----------------------------------------------------------------##
  end
  # Mail/Z-Crystals
  if GameData::Item.get(item).is_mail? || GameData::Item.get(item).is_z_crystal?
    if quantity == 1
      return _INTL("a piece of")
    else
      return _INTL("{1} pieces of",quantity)
  ...
  # Groceries
  if GameData::Item.get(item).is_grocery?
    if quantity == 1
      return _INTL("a serving of")
    else
      return _INTL("{1} servings of",quantity)
  ...
  # Pokémon materials
  if GameData::Item.get(item).is_poke_material? && GameData::Item.get(item).name == GameData::Item.get(item).name_plural
    if quantity == 1
      return _INTL("a set of")
    else
      return _INTL("{1} sets of",quantity)
    end
  end
end
...
def anSetDialogueSound(name)
	if AstralnekoConfig::DIALOGUE_SFX[name]
